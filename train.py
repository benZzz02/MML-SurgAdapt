...
def save_results(a,b,c,test,dir,method):
    ...
    if test == True:
        file_name = f"{folder_name}/result_{timestamp}_{method}_test.json"
    else:
...
def validate(trainer, epoch: int, dir) -> Tuple[float, bool]:
    ...
    loss_mean_ema = sum(losses_ema)/len(losses_ema)
    logger.info("Loss on pp regular {:.2f}, Loss on pp EMA {:.2f}".format(
        loss_mean, loss_mean_ema))
    loss_min = min(loss_mean,loss_mean_ema)
    if loss_mean_ema <= loss_mean:
        if_ema_better_loss_pp = True
    else:
        if_ema_better_loss_pp = False
    ...
    if cfg.val_sp:
        ...
        loss_mean = sum(losses)/len(losses)
        loss_mean_ema = sum(losses_ema)/len(losses_ema)
        logger.info("Loss on sp regular {:.2f}, Loss on sp EMA {:.2f}".format(
            loss_mean, loss_mean_ema))
        loss_min = min(loss_mean,loss_mean_ema)
        if loss_mean_ema <= loss_mean:
            if_ema_better_loss_sp = True
        else:
            if_ema_better_loss_sp = False
        
        evals['sp_loss'] = loss_min
...
def init_validate(trainer,epoch:int):
    ...
    trainer.model.eval()
    logger.info("Start init validation...")
    ...
    loss_mean = sum(losses)/len(losses)
    loss_mean_ema = sum(losses_ema)/len(losses_ema)
    logger.info("Loss on init val {:.2f}, Loss on init val EMA {:.2f}".format(
        loss_mean, loss_mean_ema))
    loss_min = min(loss_mean,loss_mean_ema)
    if loss_mean_ema <= loss_mean:
        if_ema_better = True
...
def train(trainer,dir) -> list:
    ...
    if cfg.perform_init:
        init_optimizer = torch.optim.Adam(
            params=parameters, lr=cfg.init_lr,
            weight_decay=0)
        ...
        trainer.model.load_state_dict(
                torch.load(f"{cfg.checkpoint}/{dir}/init/model-highest.ckpt"), strict=True)
        trainer.model.train()
...
def test_epoch_checkpoints(trainer, dir: str, epoch_ckpts) -> None:
    ...
    for epoch, reg_path, ema_path in epoch_ckpts:
        ...
        for _, (input, target, vid) in enumerate(trainer.test_loader):
            target = target.to(trainer.device, non_blocking=True)
            with torch.no_grad():
                output_reg = sigmoid(trainer.model(input.to(trainer.device, non_blocking=True)))
                output_ema = sigmoid(trainer.ema.module(input.to(trainer.device, non_blocking=True)))
    ...
    for _ in range(len(d1)):
        ...
        for i, dl_iter in enumerate(dataloader_iters):
            try:
                ...
                images, labels = next(dataloader_iters[i])
